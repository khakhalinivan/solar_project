...
class DBQuery():
    ...
    def query(self):
        ...
        print(f'start_limit: {self.start_limit}, stop_limit: {self.stop_limit}, delta: {self.stop_limit - self.start_limit}')
...
class Plot():
    ...
    def get_spectrogram_arrays(self, query):
        ...
        dep1_values = None
        print(f'type(self.variable.depend_1): {type(self.variable.depend_1)}, depend1: {self.variable.depend_1}')
        if self.variable.depend_1 is not None:
            depend_var = self.variable.dataset.variables.get_or_none(name=self.variable.depend_1)
            print(f'depend_var: {depend_var}')
            if depend_var is not None and depend_var.dynamic.exists():
                ...
                print(f'len(depend_fields): {len(depend_fields)}')
                present_depend_fields = [field_name for field_name in depend_fields if field_name in query.all_fields]
                print(f'len(present_depend_fields): {len(present_depend_fields)}')
                if len(present_depend_fields) == 1:
                    ...
                    try:
                        newline = '\n'
                        print(f'type(present_depend_fields): {type(present_depend_fields)}')
                        # print(f'present_depend_fields: {newline.join(present_depend_fields)}')
                        dep1_values = np.array(
                            [query.arrays[query.all_fields.index(field_name)][0] for field_name in present_depend_fields],
                            dtype=float,
                        )
                        print(f'try dep1_values: {dep1_values}')
                        print(present_depend_fields)
                    except Exception:
                        dep1_values = None

        y_axis = None
        print(f'dep1_values is not None: {dep1_values is not None}')
        if dep1_values is not None:
            if isinstance(dep1_values, np.ndarray) and dep1_values.ndim == 1 and dep1_values.size > 1:
                if not isinstance(dep1_values[0], (list, tuple, np.ndarray)):
                    ...
                    print(f'dep1_values: {dep1_values}')

            print(y_axis[0] is None, type(y_axis[0]), np.isnan(y_axis))
            # if y_axis is None:
        ...
        # print(f'y_axis in plot_instances.py: {y_axis}')
        if y_axis is None:
